...
def recommend1(request):
    ...
    # selectMysql()
    read_mysql_to_csv(
        "users/static/users_resulttable.csv", USERID
    )  # 追加数据，提高速率
        ...
        for i in matrix:
            ...
            for imdbId, title, poster in rr:
                ...
                if Insertposter.objects.filter(title=title):
                    ...
                    Insertposter.objects.create(
                        userId=USERID, title=title, poster=poster
                    )
...
def recommend2(request):
    ...
    # selectMysql()
    read_mysql_to_csv2(
        "users/static/users_resulttable2.csv", USERID
    )  # 追加数据，提高速率
    ...
    try:
        ...
        conn.close()
        results = Insertposter.objects.filter(
            userId=USERID
        )  # 从这里传递给html= Insertposter.objects.all()  # 从这里传递给html
...
def insert(request):
    ...
    return render(
        request, "index.html", {"userId": USERID, "rating": RATING, "imdbId": IMDBID}
    )
...
class UserBasedCF(object):
    ...
    def generate_dataset(self, filename2, pivot=1.0):
        ...
        for line in self.loadfile(filename2):
            ...
            if random.random() < pivot:  # pivot=0.7应该表示训练集：测试集=7：3
                ...
                self.trainset[user][
                    movie
                ] = rating  # trainset[user][movie]可以获取用户对电影的评分  都是整数
                trainset_len += 1
            else:
    ...
    def calc_user_sim(self):
        ...
        for (
            movie,
            users,
        ) in movie2users.items():  # 通过.items()遍历movie2users这个字典里的所有键、值
            for u in users:
                for v in users:
                    if u == v:
                        continue
    ...
    def recommend(self, user):
        ...
        for similar_user, similarity_factor in sorted(
            user_sim_mat[user].items(), key=itemgetter(1), reverse=True
        )[
            0:K
        ]:  # itemgetter(1)表示对第2个域(相似度)排序   reverse=TRUE表示降序
            for imdbid in self.trainset[
                similar_user
            ]:  # similar_user是items里面的键，就是所有用户   similarity_factor是值，就是对应的相似度
        ...
        # rank_ = dict()
        rank_ = sorted(rank.items(), key=itemgetter(1), reverse=True)[
            0:N
        ]  # 类型是list不是字典了
...
class ItemBasedCF(object):
        ...
        for m1, related_movies in itemsim_mat.items():
            for m2, count in related_movies.items():
                ...
                if simfactor_count % PRINT_STEP == 0:
                    print(
                        "calculating movie similarity factor(%d)" % simfactor_count,
                        file=sys.stderr,
                    )
    ...
    def recommend(self, user):
        ...
        for movie, rating in watched_movies.items():
            for related_movie, similarity_factor in sorted(
                self.movie_sim_mat[movie].items(), key=itemgetter(1), reverse=True
            )[:K]:
